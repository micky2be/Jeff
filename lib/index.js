...
function writeFile(outputFileName, data, options, cb) {
	//var subDir = path.dirname(outputFileName);
	//fs.mkdirsSync(subDir);
	fs.writeFile(outputFileName, data, options, cb);
}
...
Jeff.prototype._canvasToPng = function (pngName, canvas, callback) {
	...
	image.run(function (err, files) {
		...
		if (customWriteFile) {
			return customWriteFile(pngName, png, callback);
		}
		return fs.writeFile(pngName, files[0].contents, callback);
	});
};
...
*/
	if (!this._options.customWriteFile || !this._options.customWriteFile(jsonName, jsonData, JSON_WRITE_OPTIONS)) {
		return fs.writeJSON(jsonName, jsonData, { spaces: 4 }, callback);
		//return writeFile(jsonName, jsonData, JSON_WRITE_OPTIONS, callback);
	}
	callback();
};
