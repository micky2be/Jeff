...
Jeff.prototype._canvasToPng = function (pngName, canvas, callback) {
	...
	if (this._options.imageQuality < 100) {
		...
		var q = this._options.imageQuality + '-100';
		image.use(imageminPngquant({ quality: q }));
	}

	if (this._options.imageOptim) {
		//image.use(Imagemin.jpegtran());
		image.use(Imagemin.optipng());
	}

	var customWriteFile = this._options.customWriteFile;

	image.run(function (err, files) {
		if (err) {
			return callback(err);
		}
		var optiImage = files[0].contents;
		if (customWriteFile) {
			return customWriteFile(pngName, optiImage, callback);
		}
		return fs.writeFile(pngName, optiImage, callback);
	});
};
...
Jeff.prototype._dataToJson = function (jsonName, data, callback) {
	...
	if (this._options.customWriteFile) {
		return this._options.customWriteFile(jsonName, jsonData, JSON_WRITE_OPTIONS, callback);
	}
	return fs.writeFile(jsonName, jsonData, JSON_WRITE_OPTIONS, callback);
};

Jeff.prototype._extractImages = function (imageList, cb) {
	if (this._options.ignoreImages) {
		return;
	}
	var self = this;
	async.eachSeries(imageList, function (img, cb) {
		var imgName = self._generateImageName(img.name);
		fs.mkdirsSync(dirName(imgName));
		self._canvasToPng(imgName, img.img, cb);
	}, cb);
};

Jeff.prototype._extractData = function (graphicProperties, callback) {
	var dataToExport;
	var options = this._options;
	if (options.renderFrames) {
		dataToExport = helper.generateFrameByFrameData(this._symbols, this._symbolList, graphicProperties, this._options.onlyOneFrame);
	} else {
	...
	if (postProcess) {
		if (postProcess instanceof Array) {
			var nProcesses = postProcess.length;
			for (var p = 0; p < nProcesses; p += 1) {
				postProcess[p](dataToExport, this._symbols);
			}
		} else {
			postProcess(dataToExport, this._symbols);
		}
	}
	...
	if (options.flatten)          { dataToExport = helper.flattenAnimations(dataToExport); }
	if (options.simplify)         { dataToExport = helper.simplifyAnimation(dataToExport); }
	if (options.compressMatrices) { dataToExport = helper.delocateMatrices(dataToExport); }

	var jsonFileName = this._generateJsonFileName();
	this._dataToJson(jsonFileName, dataToExport, callback);
};
...
Jeff.prototype._extractClassGroup = function (imageList, graphicProperties, nextClassListCb) {
	...
	async.series([
		function (cb) {
			self._extractImages(imageList, cb);
		},
		function (cb) {
			self._extractData(graphicProperties, cb)
		}
	], nextClassListCb);
};
